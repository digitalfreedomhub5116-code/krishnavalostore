    ...
    <div className="flex flex-col overflow-hidden">
        ...
        <div className="relative z-30 text-center px-4 max-w-5xl mx-auto flex flex-col items-center md:pt-48">
          ...
          <div className="mt-16 md:mt-24 flex items-center justify-center gap-4 animate-reveal-up" style={{ animationDelay: '0.7s' }}>
            {(config.heroSlides || []).map((slide, idx) => (
              <button key={slide.id} onClick={() => setCurrentSlide(idx)} className={`group/indicator relative h-1.5 md:h-2 transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] overflow-hidden ${idx === currentSlide ? `w-12 md:w-20 skew-x-[-20deg] ${getBgClassFromAccent(slide.accent)} ${getGlowColorFromAccent(slide.accent)}` : 'w-4 md:w-6 skew-x-[-10deg] bg-white/20 hover:bg-white/40'}`}>
                {idx === currentSlide && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/60 to-transparent w-full h-full animate-shimmer"></div>}
      ...
      <section ref={procedureRef} id="how-it-works" className="relative py-24 md:py-32 bg-black overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 relative z-10">
          ...
          <div className="relative">
            <div className="absolute left-1/2 -translate-x-1/2 top-4 bottom-4 w-px md:w-1 bg-white/5 rounded-full overflow-hidden"><div className="absolute top-0 left-0 w-full bg-gradient-to-b from-brand-cyan via-brand-accent to-brand-secondary shadow-[0_0_20px_#00f0ff] transition-all duration-300" style={{ height: `${scrollProgress * 100}%` }}><div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3 h-3 md:w-6 md:h-6 bg-white rounded-full blur-[4px] md:blur-[10px] animate-pulse"></div></div></div>
      ...
      <section className="py-24 bg-black overflow-hidden border-y border-white/5 relative">
        ...
        <div className="text-center mb-16 relative z-10 animate-on-scroll reveal-up"><h2 className="text-4xl md:text-8xl font-display font-black uppercase tracking-tighter">COMMUNITY <span className="text-brand-accent glitch-text" data-text="INTEL">INTEL</span></h2><p className="text-slate-500 font-mono mt-3 uppercase tracking-[0.5em] text-[10px] md:text-sm font-bold italic">Agent Logs Transmitted From The Field</p></div>
                ...
                {review.type === 'video' ? (
                  <>
                    <div className="relative h-56 md:h-64 w-full overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center group-hover/card:bg-black/30 transition-colors"><div className="w-20 h-20 rounded-full bg-white/10 backdrop-blur-2xl border border-white/20 flex items-center justify-center pl-1.5 group-hover/card:bg-brand-accent group-hover/card:border-transparent transition-all shadow-2xl group-hover/card:scale-110"><Play className="w-8 h-8 text-white fill-white" /></div></div>
                    ...
                    <div className="p-8"><div className="flex items-center gap-5 mb-6"><div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-accent to-brand-secondary p-[1px] shadow-2xl"><div className="w-full h-full bg-brand-dark rounded-2xl flex items-center justify-center text-xl font-black text-white">{review.name.charAt(0)}</div></div><div><div className="font-black text-white font-display tracking-tight text-xl uppercase italic">{review.name}</div><div className="text-[10px] text-brand-cyan font-mono uppercase tracking-[0.3em] font-bold">SECURE CHANNEL // {review.rank}</div></div></div><p className="text-slate-400 text-sm md:text-base italic leading-relaxed font-medium">"{review.quote}"</p></div>
                  </>
                ) : (
                  <div className="p-8 flex flex-col h-full min-h-[300px] md:min-h-[350px] justify-between">
                    <div><div className="flex justify-between items-start mb-8"><div className="flex items-center gap-4"><div className="w-14 h-14 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center text-xl font-black text-slate-300 shadow-inner">{review.name.charAt(0)}</div><div><div className="font-black text-white font-display tracking-tight uppercase text-xl">{review.name}</div><div className="text-[10px] text-slate-500 font-mono uppercase tracking-widest font-bold">VERIFIED RENTER // {review.rank}</div></div></div><div className="flex gap-1 text-yellow-500 drop-shadow-[0_0_10px_rgba(234,179,8,0.4)]">{Array.from({length: review.rating || 5}).map((_, i) => <Star key={i} size={14} fill="currentColor" />)}</div></div><p className="text-slate-300 text-base md:text-lg leading-relaxed font-medium">"{review.quote}"</p></div>
                    <div className="flex items-center justify-between mt-auto pt-8 border-t border-white/5"><span className="text-[10px] font-mono text-slate-600 uppercase tracking-[0.5em] font-bold">{review.date || 'RECENT'}</span><div className="flex items-center gap-2 text-[10px] font-black text-green-400 bg-green-400/10 px-4 py-1.5 rounded-full border border-green-400/20"><CheckCircle2 size={12} /> ENCRYPTION ACTIVE</div></div>
                  </div>
                )}
              </div>
            ))}
      ...
      {selectedVideo && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-2xl animate-in fade-in duration-500 p-4"><div className="w-full max-w-6xl aspect-video rounded-none border border-brand-accent/50 relative bg-black shadow-[0_0_150px_rgba(255,70,85,0.3)]"><CustomVideoPlayer src={selectedVideo.videoUrl!} poster={selectedVideo.thumbnail} title={`INTEL DEBRIEF // AGENT: ${selectedVideo.name}`} onClose={() => setSelectedVideo(null)} /></div></div>}
